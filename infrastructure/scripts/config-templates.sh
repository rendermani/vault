...
generate_vault_job_template() {
    ...
    # Pre-compute environment variables
    local vault_addr vault_api_addr vault_cluster_addr vault_log_level
    local cpu_resources memory_resources
    
    if [[ "$environment" = "develop" ]]; then
        vault_addr="http://localhost:8200"
        vault_log_level="DEBUG"
        cpu_resources="1000"
        memory_resources="1024"
    else
        vault_addr="https://localhost:8$((200 + port_offset))"
        vault_log_level="INFO"
        if [[ "$environment" = "production" ]]; then
            cpu_resources="2000"
            memory_resources="2048"
        else
            cpu_resources="1000"
            memory_resources="1024"
        fi
    ...
    if [[ "$environment" = "develop" ]]; then
        ...
        vault_api_addr="https://vault.$DEFAULT_DOMAIN"
    fi
