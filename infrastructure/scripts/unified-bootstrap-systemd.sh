...
# Unified Infrastructure Bootstrap Script (Systemd Version)
# Handles Nomad → Vault → Traefik deployment sequence with proper systemd service management
...
usage() {
    cat <<EOF
Unified Infrastructure Bootstrap Script (Systemd Version)
Deploys Nomad → Vault → Traefik with proper systemd service management
...
Component Deployment Order:
  1. Nomad + Consul (systemd services)
...
check_prerequisites() {
    ...
    # Check if running as root (required for systemd service management)
    if [[ $EUID -ne 0 ]]; then
...
# Deploy Nomad and Consul as systemd services
deploy_nomad() {
...
validate_deployment() {
    ...
    if [[ "$DEPLOY_NOMAD" == "true" ]]; then
        log_step "Validating systemd services..."
        if [[ "$DRY_RUN" != "true" ]]; then
            if systemctl is-active --quiet consul && systemctl is-active --quiet nomad; then
                log_success "Systemd services validation passed"
            else
                log_error "Systemd services validation failed"
                validation_failed=true
            fi
        else
            log_info "[DRY RUN] Would validate systemd services"
        fi
    fi
...
DEPLOYMENT_SCRIPT_VERSION=2.0.0-systemd
...
generate_summary() {
    ...
    echo -e "${WHITE}Systemd Services:${NC} Enabled"
    ...
    if [[ "$DRY_RUN" != "true" ]]; then
        ...
        if [[ "$DEPLOY_NOMAD" == "true" ]]; then
            if systemctl is-active --quiet consul; then
                echo -e "  ${GREEN}Consul:${NC} Active (systemd)"
            else
                echo -e "  ${RED}Consul:${NC} Inactive"
            fi
            
            if systemctl is-active --quiet nomad; then
                echo -e "  ${GREEN}Nomad:${NC} Active (systemd)"
            else
                echo -e "  ${RED}Nomad:${NC} Inactive"
            fi
        fi
        ...
        if [[ "$DEPLOY_NOMAD" == "true" ]]; then
            ...
            echo -e "  ${CYAN}Consul:${NC} $CONSUL_ADDR"
        fi
        if [[ "$DEPLOY_VAULT" == "true" ]]; then
        ...
        if [[ "$ENVIRONMENT" == "develop" ]]; then
            ...
            echo -e "  ${CYAN}Consul UI:${NC} http://localhost:8500"
        ...
        echo -e "  ${CYAN}Start services:${NC} systemctl start consul nomad"
        echo -e "  ${CYAN}Stop services:${NC} systemctl stop nomad consul"
...
main() {
    ...
    log_header "UNIFIED INFRASTRUCTURE BOOTSTRAP (SYSTEMD VERSION)"
    echo -e "${WHITE}Nomad → Vault → Traefik Deployment Pipeline with Systemd Services${NC}"
    echo ""
    ...
    echo -e "  Service Management: ${CYAN}Systemd${NC}"
    echo ""
